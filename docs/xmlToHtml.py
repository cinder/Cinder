...
def gen_class_hierarchy(bs4, classDef):
    ...
    for index, base in enumerate(reversed(hierarchy)):
        ...
        if index < len(hierarchy) - 1:
            ...
            define_link_tag(a, {'href': base.fileName})
            li.append(a)
        else:
            li.append(base.name)
        ul.append(li)
...
def iterate_namespace(bs4, namespaces, tree, index, label):
    ...
    for ns in namespaces[index:]:
        ...
        aTag = gen_tag(bs4, "a")
        define_link_tag(aTag, {"href": ns.fileName})
        aTag.append(name)
...
def replace_d_tag(bs4, link):
    ...
    if link.get('kind') == 'function':
        ...
        if existingClass is not None:
            ...
            if fnObj is not None:
                new_link = gen_link_tag(bs4, link.contents[0], DOXYGEN_HTML_PATH + fnObj.path)

    elif link.get('kind') == 'enum':
        enumObj = g_symbolMap.find_enum(searchstring)
        if enumObj is not None:
            new_link = gen_link_tag(bs4, link.contents[0], DOXYGEN_HTML_PATH + enumObj.path)
    ...
    # find class link
        ...
        if existingClass is not None:
            new_link = gen_link_tag(bs4, link.contents[0], DOXYGEN_HTML_PATH + existingClass.fileName)
