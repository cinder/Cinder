...
#include "cinder/audio2/SampleRecorderNode.h"
...
{
	auto bufferPlayer = audio2::master()->makeNode( new audio2::BufferPlayerNode() );
	...
	auto connectFn = [bufferPlayer, this] {
		...
		mSamplePlayerNode >> mGain >> mPan >> audio2::master()->getOutput();
...
//	mSourceFile->setMaxFramesPerRead( 8192 );
	...
	mSamplePlayerNode = ctx->makeNode( new audio2::FilePlayerNode( mSourceFile, asyncRead ) );
...
void SamplePlayerNodeTestApp::setupBufferRecorderNode()
{
	...
	mRecorder = ctx->makeNode( new audio2::BufferRecorderNode( 2 * ctx->getSampleRate() ) );
...
{
	mSourceFile = audio2::load( dataSource, audio2::master()->getSampleRate() );
...
void SamplePlayerNodeTestApp::setupUI()
{
	...
	mEnableSamplePlayerNodeButton.mTitleNormal = "SamplePlayerNode off";
	mEnableSamplePlayerNodeButton.mTitleEnabled = "SamplePlayerNode on";
...
{
	auto bufferPlayer = dynamic_pointer_cast<audio2::BufferPlayerNode>( mSamplePlayerNode );
	if( ! bufferPlayer )
		return;
...
void SamplePlayerNodeTestApp::fileDrop( FileDropEvent event )
{
	...
	if( bufferPlayer ) {
		...
		auto filePlayer = dynamic_pointer_cast<audio2::FilePlayerNode>( mSamplePlayerNode );
