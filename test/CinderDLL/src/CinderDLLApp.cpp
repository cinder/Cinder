...
void CinderDLLApp::setup()
{
	...
	mObject = gl::Batch::create( geom::Teapot(), gl::getStockShader( gl::ShaderDef().color().lambert() ) );

	mFbo = gl::Fbo::create( getWindowWidth(), getWindowHeight(), gl::Fbo::Format().depthTexture() );
}
