...
class CameraPerspAsymTestApp : public AppBasic {
public:
...
void CameraPerspAsymTestApp::prepareSettings( Settings *settings )
{
	settings->setWindowSize(1200, 450);
}

void CameraPerspAsymTestApp::setup()
{
	...
	mCamera = CameraPerspAsym( getWindowWidth() * 0.5f, getWindowHeight(), 40.0f, 2.5f, 20.0f );
...
void CameraPerspAsymTestApp::update()
{
...
void CameraPerspAsymTestApp::draw()
{
	gl::clear(); 
	...
	gl::drawString( (boost::format("Lens Shift X: %02.2f\nLens Shift Y: %02.2f\n\nUse cursor keys to adjust lens shift,\nuse mouse to control overview camera") % mCamera.getLensShiftX() % mCamera.getLensShiftY() ).str(),
...
void CameraPerspAsymTestApp::mouseDown( MouseEvent event )
{
	mOverview.mouseDown( event.getPos() );
}

void CameraPerspAsymTestApp::mouseDrag( MouseEvent event )
{
	mOverview.mouseDrag( event.getPos(), event.isLeftDown(), false, event.isRightDown() );
}

void CameraPerspAsymTestApp::keyDown( KeyEvent event )
{
...
void CameraPerspAsymTestApp::resize( ResizeEvent event )
{
...
void CameraPerspAsymTestApp::render()
{
	// draw simple scene
	gl::color( Color(0.25f, 0.25f, 0.25f) );
	for(int i=-10; i<=10; ++i) {
...
CINDER_APP_BASIC( CameraPerspAsymTestApp, RendererGl )
