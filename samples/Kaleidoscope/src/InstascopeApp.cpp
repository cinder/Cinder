...
void InstascopeApp::defineMirrorGrid()
{
	...
	const int amtX = ceil((((WIDTH*2) - .5) / (1.5*(tri_width))) + 0.5f );
	const float w = ((amtX*1.5) + .5) * tri_width;
	const float xOffset = -(w-WIDTH)/2;
	
	const int amtY = ceil((HEIGHT*2) / (tri_height) + 0.5f );
	const float yOffset = -((amtY*(tri_height) - HEIGHT)/2);
...
void InstascopeApp::transitionMirrorIn( vector<TrianglePiece> *vec )
{
	...
	Vec2f sortPt = Vec2f(randInt(-(WIDTH/2), (WIDTH/2)), 
...
void InstascopeApp::resetSample()
{
	...
	mSamplePt.value().y = randFloat(0, WIDTH - mSampleSize);
	mSamplePt.value().x = randFloat(0, HEIGHT - mSampleSize);
	...
	do {
		newPos.x = randFloat(0, WIDTH - mSampleSize/2);
		newPos.y = randFloat(0, HEIGHT - mSampleSize/2);
		count++;
