...
layout (binding = 1) uniform usampler2D			uSamplerMaterial;
layout (binding = 2) uniform sampler2D			uSamplerNormalDepth;
layout (binding = 3) uniform sampler2D			uSamplerPosition;
layout (binding = 4) uniform sampler2DShadow 	uSamplerShadowMap;
...
void main( void )
{
	...
	if ( shadowCoord.z > uShadowBias && 
		...
		shadow = texture( uSamplerShadowMap, shadowCoord ).r;
		if ( shadow < shadowCoord.z + uShadowBias ) {
			if ( uShadowBlurSize > 0.0 ) {
				...
				for ( float i = 1.0; i < uShadowBlurSize; i += d, c += 4.0 ) {
					shadow 	+= texture( uSamplerShadowMap, shadowCoord + vec3( uShadowPixel.x, 0.0, 0.0 )	* i ).r;
					shadow 	+= texture( uSamplerShadowMap, shadowCoord + vec3( 0.0, uShadowPixel.y, 0.0 )	* i ).r;
					shadow 	+= texture( uSamplerShadowMap, shadowCoord + vec3( -uShadowPixel.x, 0.0, 0.0 )	* i ).r;
					shadow 	+= texture( uSamplerShadowMap, shadowCoord + vec3( 0.0, -uShadowPixel.y, 0.0 )	* i ).r;
				}
				shadow		/= c;
