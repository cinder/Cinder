...
class DeferredApp : public ci::app::App
{
public:
	void						draw() override;
...
void DeferredApp::draw()
{
	...
	
	{
...

		{
			...
			mGlslProgGBuffer->uniform( "uTexture",		0 );
			mGlslProgGBuffer->uniform( "uTextureMix",	0.7f );
			{
...

	{
		...
		gl::ScopedTextureBind scopedTextureBind[ 5 ] = {
			gl::ScopedTextureBind( mTextureFboGBufferAlbedo,		0 ),
			gl::ScopedTextureBind( mTextureFboGBufferMaterial,		1 ),
			gl::ScopedTextureBind( mTextureFboGBufferNormalDepth,	2 ),
			gl::ScopedTextureBind( mTextureFboGBufferPosition,		3 ),
			gl::ScopedTextureBind( mTextureFboShadowMap,			4 )
		};
...

	{
		...
		// Horizontal pass
		{
			...
			gl::ScopedTextureBind scopedTextureBind[ 2 ] = {
				gl::ScopedTextureBind( mTextureFboGBufferAlbedo,	0 ),
				gl::ScopedTextureBind( mTextureFboGBufferMaterial,	1 )
			};
			...
			mGlslProgBloom->uniform( "uTextureAlbedo",		0 );
			mGlslProgBloom->uniform( "uTextureMaterial",	1 );
		...
		// Vertical pass
		{
			...
			mGlslProgBlur->uniform( "uTexture",		0 );
			drawRect( mFboBloom->getSize() );
		}
	}
...

	{
		...
		// SSAO pass
		{
			...
			gl::ScopedTextureBind scopedTextureBind[ 2 ] = {
				gl::ScopedTextureBind( mTextureRandom,					0 ),
				gl::ScopedTextureBind( mTextureFboGBufferNormalDepth,	1 )
			};
			...
			mGlslProgSsao->uniform( "uTextureNoise",		0 );
			mGlslProgSsao->uniform( "uTextureNormalDepth",	1 );
	...
	// Set up window for composite pass
	{
		...
		gl::ScopedTextureBind scopedTextureBind[ 2 ] = {
			gl::ScopedTextureBind( mTextureFboLBuffer,		0 ),
			gl::ScopedTextureBind( mTextureFboSsaoVertical,	1 )
		};
		gl::ScopedGlslProg scopedGlslProg( mGlslProgComposite );
		mGlslProgComposite->uniform( "uTextureLBuffer",	0 );
		mGlslProgComposite->uniform( "uTextureSsao",	1 );
	...
	// Set up window for depth of field pass
	{
...

		{
			...
			mGlslProgDof->uniform( "uTexture",				0 );
			mGlslProgDof->uniform( "mTextureNormalDepth",	1 );
			gl::drawBuffer( GL_COLOR_ATTACHMENT0 );
			{
				gl::ScopedTextureBind scopedTextureBind[ 2 ] = {
					gl::ScopedTextureBind( mTextureFboComposite,			0 ),
					gl::ScopedTextureBind( mTextureFboGBufferNormalDepth,	1 )
				};
	...
	// Perform a blend pass between composite and DoF buffers
	{
		...
		gl::ScopedTextureBind scopedTextureBind[ 2 ] = {
			gl::ScopedTextureBind( mTextureFboComposite,	0 ),
			gl::ScopedTextureBind( mTextureFboDofVertical,	1 )
		};

		gl::ScopedGlslProg scopedGlslProg( mGlslProgBlend );
		mGlslProgBlend->uniform( "uBlend",		0.6f );
		mGlslProgBlend->uniform( "uTexture0",	0 );
		mGlslProgBlend->uniform( "uTexture1",	1 );
		drawRect( mFboDof->getSize() );
	}
	...
	// Set up window for color processing pass
	{
		...
		mGlslProgColor->uniform( "uTexture",		0 );
	...
	if ( mDebugMode ) {
		...
		// G-buffer
		{
			...
			gl::ScopedTextureBind scopedTextureBind[ 4 ] = {
				gl::ScopedTextureBind( mTextureFboGBufferAlbedo,		0 ),
				gl::ScopedTextureBind( mTextureFboGBufferMaterial,		1 ),
				gl::ScopedTextureBind( mTextureFboGBufferNormalDepth,	2 ),
				gl::ScopedTextureBind( mTextureFboGBufferPosition,		3 )
		...
		// Material
		{
			...
			mGlslProgDebugMaterial->uniform( "uTextureMaterial", 0 );
		...
		// Perform FXAA
		{
			...
			mGlslProgFxaa->uniform( "uTexture",	0 );
...
void DeferredApp::drawRect( const ivec2& sz )
{
	vec2 szf( sz );
	gl::ScopedModelMatrix scopedModelMatrix;
...
void DeferredApp::drawRect( const vec2& pos, const ivec2& sz )
{
	gl::ScopedModelMatrix scopedModelMatrix;
	gl::translate( pos );
	gl::scale( sz );
