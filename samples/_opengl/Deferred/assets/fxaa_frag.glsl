...
void main( void )
{
	...
	vec3 rgbUL	= texture( uTexture, vertex.uv + vec2( -1.0, -1.0 ) * uPixel ).xyz;
	vec3 rgbUR	= texture( uTexture, vertex.uv + vec2(  1.0, -1.0 ) * uPixel ).xyz;
	vec3 rgbBL	= texture( uTexture, vertex.uv + vec2( -1.0,  1.0 ) * uPixel ).xyz;
	vec3 rgbBR	= texture( uTexture, vertex.uv + vec2(  1.0,  1.0 ) * uPixel ).xyz;
	vec3 rgbM	= texture( uTexture, vertex.uv ).xyz;
	...
	vec3 color0 = 0.5 * (
		texture( uTexture, vertex.uv + dir * ( 1.0 / 3.0 - 0.5 ) ).rgb +
		texture( uTexture, vertex.uv + dir * ( 2.0 / 3.0 - 0.5 ) ).rgb );
	vec3 color1 = color0 * 0.5 + 0.25 * (
		texture( uTexture, vertex.uv + dir * -0.5 ).rgb +
		texture( uTexture, vertex.uv + dir *  0.5 ).rgb );
