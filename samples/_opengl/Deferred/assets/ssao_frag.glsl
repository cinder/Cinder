...
void main( void )
{
	float bl			= 0.0;
	float d				= 0.0;
	vec4 normalDepth	= texture( uTextureNormalDepth, vertex.uv );
	vec3 N				= normalDepth.xyz;
    float depth			= normalDepth.a;
	vec4 F				= vec4( 0.0 );
	vec3 fresnel		= normalize( ( texture( uTextureNoise, rand( vertex.uv ) * uOffset * vertex.uv ).xyz * 2.0 ) - vec3( 1.0 ) );
	vec3 R				= vec3( 0.0 );
    ...
    for ( int i = 0; i < 10; ++i ) {
		...
		F				= texture( uTextureNormalDepth, vertex.uv + sign( dot( R, N ) ) * R.xy );
		d				= depth - F.a;
